...
/**
 * Helper functions for working with OpenRouter API to access various LLMs
 */
export class OpenAiHelper {
  /**
   * Calls the OpenRouter API with the given prompts
   * @param systemPrompt - System prompt for the AI
   * @param userPrompt - User prompt for the AI
   * @param model - Model to use (from OpenRouter)
  ...
  static async callOpenAiApi(
    ...
    try {
      // Simply call OpenRouter API with the provided model
      ...
      // For now, we'll simulate a response
          ...
          // Simulate API response based on prompt content in the main method
      // This shouldn't be reached due to the provider routing above
   ...
   * @returns The AI response
   */
  static async callOpenAiDirectApi(
    systemPrompt: string,
    userPrompt: string,
    model: string,
    executeFunctions: IExecuteFunctions
    ...
    try {
      ...
      // This is how to make a real API call - will be uncommented when ready for production:
      // 
      // const credentials = await executeFunctions.getCredentials('openAiApi');
   ...
   * @returns The AI response
   */
  static async callOpenRouterApi(
    systemPrompt: string,
    userPrompt: string,
    model: string,
    executeFunctions: IExecuteFunctions
    ...
    try {
      ...
      const response = await executeFunctions.helpers.httpRequest({
        ...
        headers: {
          'Authorization': `Bearer ${apiKey}`,
          'Content-Type': 'application/json',
          'HTTP-Referer': 'https://n8n.io', // Required by OpenRouter
          'X-Title': 'N8N AI Assistant',     // Optional - shows in OpenRouter logs
        },
        body: {
          model: model,
          messages: [
      ...
      // If API call fails, fall back to mock responses for development
      console.log('OpenRouter API call failed, using mock response:', error.message);
      
      // Add OpenRouter model info to the response
  ...
  /**
   * Generates a simulated analysis response
...
`;
  ...
  /**
   * Generates a simulated optimization response
   * @param prompt - The user prompt
   * @returns Simulated AI response
   */
...
`;
  ...
  /**
   * Generates a simulated explanation response
   * @param prompt - The user prompt
   * @returns Simulated AI response
   */
  static generateExplanationResponse(prompt: string): string {
    // Extract node type from prompt if available
...
`
  ...
  /**
   * Generates a simulated workflow generation response
   * @param prompt - The user prompt
   * @returns Simulated AI response
   */
...
`;
  ...
  /**
   * Generates a simulated general response
   * @param prompt - The user prompt
   * @returns Simulated AI response
   */
